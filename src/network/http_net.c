/*
 * @file http_net.c
 * @brief HTTP server layer for the ImgFS project.
 *
...
static void *handle_connection(void *arg)
{
    ...
    int content_len = 0; 
    size_t max_read = MAX_HEADER_SIZE;
    ...
    while (1) {
        ...
        if (parsed == 0 && content_len > 0 &&
            total_bytes_read < (size_t)content_len && !already_extended) {
...
int http_reply(int connection, const char* status, const char* headers, const char *body, size_t body_len) {
    ...
    if ((size_t)bytes_sent != total_size) {
        perror("send error");
        return ERR_IO;
    }
